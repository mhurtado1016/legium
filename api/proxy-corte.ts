...
// solos; Deno (donde corren las Edge Functions de Supabase) no, y
// Deno.createHttpClient({ caCerts }) tiene problemas conocidos en el
// runtime de Supabase (no vainilla Deno). Node.js sí maneja certificados
// personalizados de forma confiable vía https.Agent, así que este
// endpoint —en la propia infraestructura de Legium, sin depender de
// ningún tercero— hace la conexión real y le devuelve el resultado a
// la Edge Function que lo pida.
...
// https://certs.godaddy.com/repository/gd_bundle_dv-r1-g2.crt.pem
// y verificada con `openssl verify`. Si GoDaddy la rota (vigencia del
// intermedio hasta 2027), hay que repetir el proceso y reemplazar esto.
